    ...
    if event == "log-in":
        if values[0] and values[1]:
            if values[0] == "king" and values[1] == "1234":
                lg.logger.info("open capacity window")
                return True
            else:
...
def validate_input_car_details(values: sg) -> bool:
    ...
    if values["TYPE"] not in ["Bike", "Car", "Bus"]:
        sg.popup("Enter types of Bike, Bus, Car  ", title="Error input", font=(20, 20))
        lg.logger.warning("Enter types of Bike, Bus, Car ")
        return False
...
def check_events_car_and_reports(
    ...
    """
    if event == "Submit" and validate_input_car_details(values):
