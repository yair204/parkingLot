...
def main_window():
    sg.theme("BlueMono")
    layout = [[
        sg.Button("Add car", size=(6, 4)),
        sg.Button("Remove car", size=(6, 4)),
        sg.Button("Reports", size=(6, 4)),
        sg.Button("Check capacity", size=(6, 4)),
        sg.Button("Add gate", size=(6, 4)),
     ] ,[sg.Push()],
        [sg.Button("Exit")],
              ]
...
def run_gui() -> None:
    ...
    while True:  # Event Loop
        ...
        if event == "Add car" and not add_car_window:
            list_windows[3] = window_add_car()

        if event == "Remove car" and not remove_car_window:
